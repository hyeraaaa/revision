...
{
    global $pdo;
    $s_contact_number = "+63" . $s_contact_number;
    ...
    $checkCNQuery = "SELECT COUNT(*) FROM student WHERE contact_number = :contact_number";
    $checkStmt = $pdo->prepare($checkCNQuery);
    $checkStmt->bindParam(':contact_number', $s_contact_number);
    $checkStmt->execute();

    $contactNumberExists = $checkStmt->fetchColumn();

    if ($contactNumberExists > 0) {
        echo "<script>alert('Error: This contact number is already registered.');
        window.location.href = 'manage_student.php';</script>";
        exit;
    } else {
        // Check if email exists in the admin table
        $checkCNQuery = "SELECT COUNT(*) FROM admin WHERE contact_number = :contact_number";
        $checkStmt = $pdo->prepare($checkCNQuery);
        $checkStmt->bindParam(':contact_number', $s_contact_number);
        $checkStmt->execute();
    ...
    if ($stmt->execute()) {
        ...
        $mail->SMTPSecure = PHPMailer::ENCRYPTION_STARTTLS; // StartTLS encryption (safer and faster than SSL)
        $mail->Port = 587;
        $mail->SMTPKeepAlive = true; // Keep connection alive for faster sending in bulk
        $mail->SMTPDebug = 0; // Disable debugging to improve performance
